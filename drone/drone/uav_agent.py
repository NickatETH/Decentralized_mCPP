...
class UavAgent(Node):
    ...
    def __init__(self, agent_id, position, boundary):
        ...
        self._energy_srv = self.create_service(
            ComputeEnergy, "~/compute_energyy", self.compute_energy_cb
        )
    ...
    def _send_test(self, rid):
        ...
        pkt = Float32MultiArray(
            data=[0, rid, self.agent_id, nbr, self.frag.level, self.frag.frag_id, w]
        )
    ...
    def ghs_cb(self, msg: Float32MultiArray):
        ...
        if typ == 0:  # TEST
            ...
            pkt = Float32MultiArray(
                data=[ans_typ, rid, self.agent_id, src, lvl, self.frag.frag_id, weight]
            )
            self.ghs_pub.publish(pkt)
            if accept:
